...
{
  unsigned char old_val = state->reg_a;
  state->stack_pointer = value;
  char *buffer = malloc(19);
  snprintf(buffer, 19, "A: $%02x -> $%02x", old_val, state->reg_a);
  return buffer;
}
...
{
	bool old_val = state->z_flag;
  state->z_flag = value;
  char *buffer = malloc(19);
  snprintf(buffer, 19, "Z flag: %d -> %d", old_val, state->z_flag);
  return buffer;
}
